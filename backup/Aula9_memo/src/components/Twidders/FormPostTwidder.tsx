import React, { RefObject, SyntheticEvent, useRef, memo } from "react";
  ...
  return (
    <div>
      <form onSubmit={(event: SyntheticEvent) => handleCreatePost(event, textarea)}>
        <label style={{ display: "block" }} htmlFor="twidderMessage">
          Twidde agora:
        </label>
        <textarea
          ref={textarea}
          style={{ fontFamily: "sans-serif", fontSize: "1.3rem", border: "1px solid #DDD" }}
          id="twidderMessage"
          cols={120}
          rows={2}
        ></textarea>
...
export default memo(FormPostTwidder);

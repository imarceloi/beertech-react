import React, { RefObject, SyntheticEvent, useEffect, useState, useMemo, useCallback } from "react";
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      const newMessages = await messages.map(async (message) => {
        const { data: remoteComments } = await Axios.get<null, CommentsRemote>(
          `https://jsonplaceholder.typicode.com/posts/${message.id}/comments`
        );
  ...
  return (
    <section style={{ fontFamily: "sans-serif", fontSize: "1.3rem" }}>
      <div style={style}>
        ...
        <button onClick={() => handleChangePost()}>Change Message 1</button>
        {twidders.length > 0
          ? twidders.map(({ id, title, comments }) => (
